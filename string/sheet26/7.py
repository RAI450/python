# 7: Advanced Smart Chat Compression Expansion System
...
n=input("Enter compressed message: ").lower()

l=len(n)
msg=""
for i in range(l):
    if (ord(n[i])>=97 and ord(n[i])<=122) or (ord(n[i])>=48 and ord(n[i])<=57):
        if ord(n[i])>=48 and ord(n[i])<=57:
            msg=msg+(n[i-1])*(int(n[i])-1)
        else:
            msg=msg+n[i]
print("Expanded Message: ",msg)
